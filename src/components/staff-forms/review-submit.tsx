...
interface ReviewSubmitProps {
  ...
  isStaffMode?: boolean; // NEW: Flag to indicate staff mode
}
  ...
  const handleSubmit = async () => {
    try {
      ...
      const studentName = user?.firstName && user?.lastName 
        ...
        : accInfo?.Name || (isStaffMode ? 'Staff Member' : 'Student');
      ...
      if (isStaffMode) {
        // Staff submission - goes directly to utilization reservation API
        const submissionData = {
          days: formData.days,
          ProductsManufactured: formData.ProductsManufactured,
          BulkofCommodity: formData.BulkofCommodity || 'Staff Equipment Request',
          Equipment: formData.Equipment,
          Tools: formData.Tools,
          Remarks: 'Staff equipment reservation - direct admin approval',
          
          // Staff defaults for required fields
          LvlSec: 'N/A',
          NoofStudents: 0,
          Subject: 'N/A',
          Teacher: 'N/A',
          TeacherEmail: 'N/A',
          Topic: 'N/A',
          SchoolYear: formData.SchoolYear || new Date().getFullYear(),
        ...
        // STUDENT MODE: Original EVC submission logic
        const submissionData = {
          ...
          <CardContent className="pt-4">
            <div className="flex flex-col items-center justify-center py-12">
              ...
              <h3 className="text-xl font-semibold text-blue-800 mb-2">
                {isStaffMode ? 'Processing Staff Reservation' : 'Processing Your Reservation'}
              </h3>
    ...
    <div className="w-full max-w-6xl mx-auto px-2 sm:px-4 pt-0 flex flex-col">
          ...
          <div className="flex items-start">
            ...
            <div>
              ...
              <p className="text-blue-700 text-sm mt-1">
                ...
                This is a staff equipment reservation and will be processed accordingly.
              </p>
            </div>
          </div>
        </div>
      ...
      <Card className="bg-white shadow-sm border border-gray-200 mt-6">
        <CardContent className="pt-0">
          ...
          {formData.NeededMaterials && formData.NeededMaterials.length > 0 && (
            <div className="mb-6">
              <h3 className="text-lg font-medium mb-3 flex items-center">
                ...
                {isStaffMode ? 'Equipment Requested' : 'Materials Needed'}
              </h3>
              <Card className="border-gray-200 shadow-none">
                <CardContent className="p-4">
          ...
          {!isStaffMode && !formData.TeacherEmail && (
          ...
          <div className="mt-6 flex justify-between">
            ...
            <Button
              ...
              disabled={isSubmitting || loading || (!isStaffMode && !formData.TeacherEmail)}
              className="bg-blue-600 hover:bg-blue-700 text-white transition-colors disabled:opacity-50"
            >
              {isSubmitting ? 'Submitting...' : (isStaffMode ? 'Submit Equipment Request' : 'Submit Request')}
            </Button>
          </div>
        </CardContent>
      </Card>
