...
interface LabReservationProps {
  ...
  nextStep: () => void;
  prevStep: () => void;
  userRole?: string; // ❌ Make sure this matches
}
...
export default function LabReservation({ 
  ...
  useEffect(() => {
    if (isStaff && !hasInitialized.current) {
  ...
  }, [isStaff, formData, updateFormData, currentYear]);
  ...
  useEffect(() => {
    if (isStaff) return; // Skip for staff mode
    ...
    } else if (students.length === 0 && !isStaff) {
      // If no user is loaded yet but we need at least one student entry (students only)
      addStudent();
    }
  }, [isLoaded, user, students, updateFormData, addStudent, isStaff]);
  ...
  const updateStudentName = (index: number, name: string) => {
    if (isStaff) return; // Disable for staff mode
  ...
  const removeStudent = (index: number) => {
    if (isStaff || students.length <= 1) return; // Disable for staff mode
    ...
    <div className="max-w-4xl mx-auto my-8 px-4">
      ...
      <div className="bg-white p-8 rounded-lg shadow-md border border-gray-200">
        <h2 className="text-2xl font-bold mb-6 text-blue-700 pb-3 border-b border-gray-200">
          {isStaff ? 'Lab Equipment Reservation' : 'Laboratory Reservation'}
        </h2>
          ...
          <section className="mb-10">
            <div className="flex items-center mb-5">
              ...
              <h3 className="text-lg font-semibold text-gray-800">
                {isStaff ? 'Administrative Information' : 'Class Information'}
              </h3>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  <label htmlFor="lvlSec" className="block text-sm font-medium mb-1 text-gray-700">
                    Level/Section {!isStaff && <span className="text-red-500">*</span>}
                  </label>
                  <input
                    id="lvlSec"
                    type="text"
                    className={`w-full border ${errors.LvlSec ? 'border-red-500' : 'border-gray-300'} rounded-lg p-3 focus:ring-2 focus:ring-blue-300 focus:border-blue-500 outline-none transition ${isStaff ? 'bg-gray-100' : ''}`}
                ...
                <div>
                  <label htmlFor="subject" className="block text-sm font-medium mb-1 text-gray-700">
                    Subject {!isStaff && <span className="text-red-500">*</span>}
                  </label>
                  <input
                    id="subject"
                    type="text"
                    className={`w-full border ${errors.Subject ? 'border-red-500' : 'border-gray-300'} rounded-lg p-3 focus:ring-2 focus:ring-blue-300 focus:border-blue-500 outline-none transition ${isStaff ? 'bg-gray-100' : ''}`}
                    value={formData.Subject}
                    onChange={(e) => handleFieldChange('Subject', e.target.value)}
                    aria-invalid={!!errors.Subject}
                    placeholder={isStaff ? "N/A (Staff)" : "e.g. Chemistry"}
                    readOnly={isStaff}
                  />
                ...
                <div>
                  <label htmlFor="teacher" className="block text-sm font-medium mb-1 text-gray-700">
                    Teacher {!isStaff && <span className="text-red-500">*</span>}
                  </label>
                  <input
                    id="teacher"
                    type="text"
                    className={`w-full border ${errors.Teacher ? 'border-red-500' : 'border-gray-300'} rounded-lg p-3 focus:ring-2 focus:ring-blue-300 focus:border-blue-500 outline-none transition ${isStaff ? 'bg-gray-100' : ''}`}
                    value={formData.Teacher}
                    onChange={(e) => handleFieldChange('Teacher', e.target.value)}
                    aria-invalid={!!errors.Teacher}
                    placeholder={isStaff ? "N/A (Staff)" : "e.g. Cyrill Tapales"}
                    readOnly={isStaff}
                  />
                ...
                <div>
                  <label htmlFor="teacherEmail" className="block text-sm font-medium mb-1 text-gray-700">
                    Teacher Email {!isStaff && <span className="text-red-500">*</span>}
                  </label>
                  <div className="relative">
                    <input
                      id="teacherEmail"
                      type="email"
                      className={`w-full border ${errors.TeacherEmail ? 'border-red-500' : emailVerificationStatus === 'valid' ? 'border-green-500' : 'border-gray-300'} rounded-lg p-3 focus:ring-2 focus:ring-blue-300 focus:border-blue-500 outline-none transition pr-10 ${isStaff ? 'bg-gray-100' : ''}`}
                      ...
                      onBlur={(e) => !isStaff && verifyTeacherEmail(e.target.value)}
                      aria-invalid={!!errors.TeacherEmail}
                      placeholder={isStaff ? "N/A (Staff)" : "e.g. teacher@evc.pshs.edu.ph"}
                      readOnly={isStaff}
                    />
                  ...
                  {!isStaff && emailVerificationStatus === 'valid' && !errors.TeacherEmail && (
                ...
                <div>
                  <label htmlFor="topic" className="block text-sm font-medium mb-1 text-gray-700">
                    Topic {!isStaff && <span className="text-red-500">*</span>}
                  </label>
                  <input
                    id="topic"
                    type="text"
                    className={`w-full border ${errors.Topic ? 'border-red-500' : 'border-gray-300'} rounded-lg p-3 focus:ring-2 focus:ring-blue-300 focus:border-blue-500 outline-none transition ${isStaff ? 'bg-gray-100' : ''}`}
                    value={formData.Topic}
                    onChange={(e) => handleFieldChange('Topic', e.target.value)}
                    aria-invalid={!!errors.Topic}
                    placeholder={isStaff ? "N/A (Staff)" : "e.g. Acid-Base Reactions"}
                    readOnly={isStaff}
                  />
