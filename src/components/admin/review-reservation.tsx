...
const ReviewReservation: React.FC<ReviewReservationProps> = ({
  ...
  // Fix for handleApproveReservation function
const handleApproveReservation = async () => {
  ...
  try {
    ...
    const operatingTimes = localReservation.UtilTimes.map(time => {
      ...
      return {
        OTDate: startTime ? new Date(startTime).toISOString() : null, // Keep full ISO string with time part
    ...
    if (machineUtilizations.length > 0) {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          throw new Error(errorText || 'Failed to create machine utilization records');
        }
      }
      
      // Parse the response as JSON only if response was ok
