 ...
 * @param utilizationData - The machine utilization data
 */
export const downloadJobPaymentOrderPDF = (utilizationData: MachineUtilizationData): void => {
  try {
    ...
    try {
      // Left logo - PSHS Logo
      const leftLogoUrl = '/images/logos/pshs_logo.png';
      ...
      doc.text('PSHS', margin + 10, 22, { align: 'center' });
    }
    ...
    const operatingHeaders: AutoTableColumnOption[] = [
    ...
    const operatingRows: AutoTableColumnOption[][] = [];
    ...
    try {
      ...
      yPosition = result.endY || (yPosition + 45);
    } catch (error) {
    ...
    // Downtimes Table
    const downtimeHeaders: AutoTableColumnOption[] = [
      { content: 'Date', styles: { fontStyle: 'bold', halign: 'center' } },
      { content: 'Type of products processed', styles: { fontStyle: 'bold', halign: 'center' } },
    ...
    const downtimeRows: AutoTableColumnOption[][] = [];
    ...
    try {
      ...
      yPosition = result.endY || (yPosition + 40);
    } catch (error) {
    ...
    // Prepare repair data
    const repairRows: AutoTableColumnOption[][] = [];
    ...
    // Review footer
    const reviewTableData: AutoTableColumnOption[][] = [
      [
    ...
    doc.save(`Machine_Utilization_Form_${formatDate(new Date().toISOString())}.pdf`);
  } catch (error) {
...
// Sample usage function to demonstrate how to call the PDF generator
export const generateMachineUtilizationPDF = (): void => {
  // Sample data
  ...
  downloadJobPaymentOrderPDF(sampleData);
};